...
Namespace Qportals
    Public Class QPortal
        Public Sub New()
        End Sub
        Public Function remove_quarter(ByRef strText As String)
...
#End Region
    ...
    Public Class External_library
        ...
        Public Class EPPlus
            ...
            ''' <summary>
            ''' datatable을 excel로 변형 합니다.
            ''' </summary>
            ''' <param name="_dt"> 데이터테이블 입니다. </param>
            ''' <param name="fileName"> export할 경로 입니다. </param>
...
#Region "</Summary> Then   </Summary>"
            ''' <summary>
            ''' 엑셀 열고 데이터를 Datatable로 가져오기
            ''' </summary>
            ...
            Public Function ReadAllxlsm_set_datatable(ByRef path As String, ByRef shtName As String) As DataTable
                ...
                Using p As New ExcelPackage(isExist)
                    ...
                    For Each cell In ms.Cells(9, 2, 9, ms.Dimension.End.Column)
...
#End Region
    ...
    Public Class DbConnection
        ...
        Public Class Excel_Class
            ...
            Private Function Excel_to_datatable(ByRef path As String, ByRef Sheet As String, ByRef sql As String) As DataTable
        ...
        ''' <summary>
        ''' 엑셀 파일을 DataTable로 Return 하는 함수 입니다.
        ''' </summary>
        ''' <param name="path"> ex) "c://test\test" </param>
        ''' <param name="range"> ex) "A2:C10" </param>
        ''' <param name="HeaderYN"> YES or NO </param>
        ''' <returns></returns>
        Public Function Excel_to_datatable(ByRef path As String, ByRef range As String, Optional HeaderYN As String = "YES") As DataTable
            Dim output_dt As DataTable = New DataTable
            ...
            vConn = New System.Data.OleDb.OleDbConnection("Provider=Microsoft.Ace.OLEDB.12.0;Data Source=" & path & ";Extended Properties=""Excel 12.0;HDR=" & HeaderYN & ";""")
            Try
                vConn.Open()
            Catch ex As Exception
...
#End Region
        ...
        Public Class Mysql_Class
            ...
            Public Function Query_to_Mysql(ByRef sql As String, Optional ByRef connstring As String = "Server=59.16.241.6;Uid=sunbae89;Pwd=sunbae;Database=td_defect") As Boolean
...
#Region "<Summary> mysql to Datatable </Summary>"
            Public Function Mysql_to_datatable(ByRef sql As String, Optional ByRef connstring As String = "Server=59.16.241.6;Uid=sunbae89;Pwd=sunbae;Database=td_defect") As DataTable
                Dim dt As New System.Data.DataTable
                ...
                Using cn As New MySql.Data.MySqlClient.MySqlConnection With {.ConnectionString = connstring}
                    Using cmd As New MySql.Data.MySqlClient.MySqlCommand With {.Connection = cn}
                        ...
                        Using da As New MySql.Data.MySqlClient.MySqlDataAdapter(cmd)
                            Try
                                da.Fill(dt)
                                Debug.Print("[Query result] > " & "Successfull" & sql)
                            Catch ex As Exception
...
#Region "<Summary> Return Query Result of String </Summary>"
            Public Function GetQueryResult(ByRef sql As String) As String
                Dim _server_address As String = c_db._server_address
                Dim result As String = Nothing
                Using cn As New MySql.Data.MySqlClient.MySqlConnection With {.ConnectionString = _server_address}
                    Using cmd As New MySql.Data.MySqlClient.MySqlCommand With {.Connection = cn}
                        cmd.CommandType = System.Data.CommandType.Text
                        cmd.CommandText = sql
                        cn.Open()
...
#End Region
            ...
            Public Function GetContacts(ByRef user As String) As DataTable
                ...
                Dim connString As String = "Server=59.16.241.6;Uid=sunbae89;Pwd=sunbae;Database=" & "td_defect"
                Dim sql As String
    ...
    Public Class Controls
        ...
        Public Class ListBoxClass
            ...
            Public Property _textboxs() As TextBox
            ...
            Public Sub ClickItems(ByRef lstBoxs() As ListBox, ByRef _dt As DataTable)
                ...
                Select Case lstBoxs.Length
                    ...
                    Case 3
                        ...
                        If _dt.Columns.Contains("모델리더") Then
                            For Each dr As DataRow In _dt.Rows
        ...
        Public Class TreeListViewMaker
            ...
            Public Overloads Sub BuildTree(ByRef _dt As DataTable, columns() As Integer, Optional expandAll As Boolean = False)
                ...
                For Each row As DataRow In _dt.Rows
                    ...
                    If (node Is Nothing) Or (columnIndex < _columns.Length) Then '// 0 < 1
            ...
            Private Function SearchNode(parent As TreeListNode, row As DataRow, ByRef columnIndex As Integer) As TreeListNode
                ...
                For Each item As TreeListNode In nodes
                    ...
                    End If
                Next
                '// 노드가 있었다면 노드를 return 해주는데 없으면 nothing
            ...
            Private Sub AddNode(ByRef parent As TreeListNode, row As DataRow, ByRef columnIndex As Integer)
                ...
                If parent Is Nothing Then
                    ...
                    columnIndex = columnIndex + 1
                    '// 나중에 columnIndex로 몇 번째 column에 subitem을 넣을지 판단 가능
                End If
                ...
                For Each row As DataRow In _dt.Rows
                    ...
                    If Not IsDBNull(row.Item(0)) Then
                        If row.Item(0).ToString <> nodeName_1 Then
                            ...
                            Trv.Nodes.Item(Trv.Nodes.Count - 1).Font = New Font("맑은 고딕", 7.25)
            ...
            Public Sub Make_Node_Member(ByRef _dt As DataTable, ByVal Trv As TreeListView, Optional Collab As Boolean = False)
                ...
                For Each row As DataRow In _dt.Rows
                    ...
                    If Not IsDBNull(row.Item(2)) Then
                        If row.Item(2).ToString <> nodeName_3 Then
                            ...
                            node_2.Nodes.Add(node_3)
                            node_2.Nodes.Item(node_2.Nodes.Count - 1).SubItems.Add(row.Item(3).ToString)  ' 이름 ex) 이순배
                            node_2.Nodes.Item(node_2.Nodes.Count - 1).SubItems.Add(row.Item(4).ToString)  ' 이름 ex) 이순배
                        End If
            ...
            Public Sub Make_Node_Member_node(ByRef _dt As DataTable, ByVal Trv As TreeListView, Optional Collab As Boolean = False)
            ...
            Public Sub BuildList(ByRef listview As ListView, ByRef dt As DataTable, ByRef columns() As Integer)
                ...
                For Each col As DataColumn In _dtable.Columns
                    If _listview.InvokeRequired Then
                        _listview.Invoke(Sub() _listview.Columns.Add(col.ColumnName.ToString))
                    Else
                ...
                For i As Integer = 0 To _dtable.Rows.Count - 1
                    ...
                    If _listview.InvokeRequired Then
                        _listview.Invoke(Sub() _listview.Items.Add(lvi))
                    Else
        ...
        Public Class TreeViewMaker
            ...
            Private Sub AddNode(ByRef parent As TreeNode, row As DataRow, ByRef columnIndex As Integer)
                If parent Is Nothing Then
                    'If row(_columns(0)).ToString() = "교육" Or 
...
#Region " get User Name "
        Public Function getUserName() As String
            ...
            Next
            Dim strTemps As String = strName
            strName = Strings.Split(strName, "/")(0)

            'Dim strtemp() As String = Strings.Split(strTemps, ";")
        ...
        Public Function getEmail() As String
            ...
            Dim strtemp As String = Strings.Split(strName, ";")(1)
...
#End Region
    ...
    Public Class Debugging
        ...
        End Function
        ''' <summary>
        ''' buttonOption1 : Error(16),Information(64), Question(32)
        ''' </summary>
        ''' <param name="s"></param>
        ''' <param name="title"></param>
        ''' <param name="buttonOption1"></param>
